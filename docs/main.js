...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const bookingButtonFooter = document.getElementById('booking-button-footer');
    const closeModalButton = document.getElementById('booking-modal-close');
    ...
    function renderTimeSlots(startDate, endDate, busySlots) {
        ...
        for (let day = new Date(startDate); day < endDate; day.setDate(day.getDate() + 1)) {
            ...
            for (let hour = businessHoursStart; hour < businessHoursEnd; hour++) {
                ...
                const isBusy = busyIntervals.some(busy => (slotStart < busy.end && slotEnd > busy.start));
                if (!isBusy) {
